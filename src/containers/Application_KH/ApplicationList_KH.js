...
function ApplicationList_KH() {
  ...
  useEffect(() => {
    ...
    localStorage.removeItem(FILTER_STORAGE_KEY_KH);
    localStorage.removeItem(STATE_STORAGE_KEY_KH);
  ...
  const fetchApplications = async (
    ...
    try {
      ...
      const payload = {
        ...
        // ✅ CÁI BẠN CẦN: nhận ngay giá trị 1 khi bấm dashboard
      ...
      console.error("Lỗi khi lấy danh sách đơn đăng ký (KH):", error);
      setApplications([]);
      setTotalItems(0);
    } finally {
      setLoading(false);
