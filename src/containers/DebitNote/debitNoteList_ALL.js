...
function DebitNoteList_ALL() {
    ...
    const allFieldOptions = [
        ...
        { label: "Trạng thái thanh toán", labelEn: "Payment status", key: "trangThaiThanhToan" },
    ...
    const fetchDebitNote = async (searchValue, partnerId, countryId, customerId, casetypeId, staffId, page = 1, size = 10) => {
        ...
        try {
            ...
            const response = await callAPI({
                method: "post",
                endpoint: "/denghithanhtoan_all/list",
                data: {
                    ...
                    maNhanSu: staffId,
                    fields: selectedFields.filter(f => f !== "trangThaiThanhToan"),
                    pageIndex: page,
                    pageSize: size,
                },
            });
                                ...
                                debitNotes.map((debitNoteItem, index) => (
                                    <tr key={debitNoteItem.id} className="group hover:bg-gray-100 text-center border-b relative">
                                        ...
                                        {columns.map((col, colIndex) => {
                                            ...
                                            if (
                                                col.key === "ngayThanhToan" ||
                                                col.key === "ngayGuiHoaDon" ||
                                                col.key === "ngayXuat"
                                            ) {
                                                content = content ? new Date(content).toLocaleDateString("vi-VN") : "";
                                            }
                                            if (col.key === "trangThaiThanhToan") {
                                                const ngayTT = debitNoteItem.ngayThanhToan;
                                                const isPaid = ngayTT !== null && ngayTT !== "" && ngayTT !== undefined;
...
export default DebitNoteList_ALL;
